...
print ("${voffset 2}${color0}${font Pie charts for maps:size=14}"+icon+"${font}${color}   ${voffset -5}Root: ${color}${alignr}${color2}${fs_free /}${color}${font}\n")
...
if os.path.ismount("/home"):
  ...
  print ("${voffset -7}${color0}${font Pie charts for maps:size=14}"+icon+"${font}${color}   ${voffset -5}Home: ${color}${alignr}${color2}${fs_free /home}${color}${font}\n")
...
for device in os.listdir("/media/"):
  if (not device.startswith("cdrom")) and (os.path.ismount('/media/'+device)):
    ...
    if dec > 9:
      ...
      icon = str(dec)
    # end calculation dec
    # print ("${voffset -6}${color0}${font Pie charts for maps:size=14}"+icon+"${font}${color}   ${voffset -5}"+device[0:10].capitalize()+": ${font Ubuntu:style=Bold:size=8}${color1}${fs_free_perc /media/"+device+"}%${color} ${alignr}${color2}${fs_free /media/"+device+"}${color}${font}\n")
    short_device = device[0:10] + (u"..." if len(device) > 11 else ':')
    print ("${voffset -7}${color0}${font Pie charts for maps:size=14}"+icon+"${font}${color}   ${voffset -5}"+short_device.capitalize()+" ${color}${alignr}${color2}${fs_free /media/"+device+"}${color}${font}\n")
print ("${voffset -6}")
